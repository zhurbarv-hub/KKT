...
>
> Последнее обновление: 2026-02-10 (v3 — setup page, настраиваемое название, seed данных)
...
│   └── setup.py         # Первоначальная настройка + системные настройки
└── utils/
...
|------|----------|--------|
| `/login` | LoginPage | Публичный (редирект на /setup если не настроена) |
| `/setup` | SetupPage | Публичный (только при первом запуске) |
...
#### system_settings
Ключ-значение системных настроек.
...
|------|----------|----------------------|
| `company_name` | Название компании (отображается в Sidebar и на странице входа) | "KKT System" |
...
### Первоначальная настройка (`/api/setup`)
| Метод | Путь | Описание | Аутентификация |
|-------|------|----------|----------------|
| GET | `/api/setup/status` | Нужна ли настройка (`{needs_setup: bool}`) | Нет |
| GET | `/api/setup/settings` | Публичные настройки (`{company_name: str}`) | Нет |
| POST | `/api/setup` | Создание первого админа + название компании | Нет (403 если уже настроена) |
...
### Первоначальная настройка (Setup Flow)

При первом запуске на новом сервере:

1. **Backend startup** (`main.py → startup_event`):
   - Создаёт таблицы БД (`init_db()`)
   - Сидирует системные типы дедлайнов: «Замена ФН» (id=6) и «Продление договора ОФД» (id=7)
   - Сидирует 15 ОФД-операторов (если таблица пуста)
   - Сдвигает PG sequence `deadline_types_id_seq` за id=7 чтобы избежать конфликтов

2. **Пользователь открывает сайт**:
   - `/login` → проверяет `GET /api/setup/status` → если `needs_setup: true` → редирект на `/setup`
   - На странице `/setup` вводит: **название компании**, ФИО, email, пароль
   - `POST /api/setup` → создаёт администратора и сохраняет `company_name` в `system_settings`
   - Редирект на `/login` → вход с созданными данными

3. **Sidebar и LoginPage** загружают название из `GET /api/setup/settings`

> **Важно:** Endpoint `POST /api/setup` работает ТОЛЬКО при 0 пользователей в БД. После создания первого пользователя возвращает 403.

### Seed данных при старте

| Данные | Условие | Количество |
|--------|---------|------------|
| Типы дедлайнов (is_system=True) | Если id=6 или id=7 не существуют | 2 типа |
| ОФД-операторы | Если таблица пуста | 15 операторов |

Системные типы имеют фиксированные id (6 и 7), так как они захардкожены в `cash_registers.py` (`FN_DEADLINE_TYPE_ID`, `OFD_DEADLINE_TYPE_ID`) для автоматического создания дедлайнов при добавлении ККТ.

### Настраиваемое название компании

- Хранится в `system_settings` с ключом `company_name`
- Устанавливается при первоначальной настройке (Setup Page)
- Отображается в: Sidebar (заголовок + буква логотипа), LoginPage (заголовок + подпись)
- Для уже настроенных систем можно задать вручную:
  ```sql
  INSERT INTO system_settings (key, value) VALUES ('company_name', 'Моя компания')
  ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value;
  ```

### Watchtower (автообновление Docker-образов)

Watchtower запускается на клиентских серверах (профиль `client`) и автоматически обновляет контейнеры при появлении новых образов в ghcr.io.
...
watchtower:
  ...
  environment:
    ...
    DOCKER_API_VERSION: "1.44"           # обязательно для Docker 27+
  command: kkt-backend kkt-bot kkt-nginx  # только эти контейнеры
  profiles: [client]
```

> **DOCKER_API_VERSION=1.44** — без этой переменной Watchtower падает в crash loop на Docker 27+ (`client version 1.25 is too old. Minimum supported API version is 1.44`).

### Валидация данных

Все сообщения валидации в `backend/schemas.py` — на русском языке:
- ИНН: «ИНН должен содержать только цифры», «ИНН должен содержать 10 или 12 цифр»
- Телефон: «Телефон должен быть в формате: +7XXXXXXXXXX или 8XXXXXXXXXX»
- Роль: «Роль должна быть: client, manager или admin»
- Пароль: «Пароль обязателен для менеджеров и администраторов»
- Дни уведомлений: «Укажите хотя бы один день уведомления», «Дни уведомлений должны быть от 1 до 365»
...
# 🚨 Инцидент безопасности 08.08.2026 — угон Telegram-бота

## Что произошло
Бот `@your_kkt_bot` начал рассылать рекламу казино и сменил имя на
«BEST CASINO MINI-APP @Xstakerobot». Причина — **угон токена бота**.

Что успел сделать злоумышленник, имея только токен:
- поставил свой **webhook** `https://ssh.inkognit.org:8443/hook/...` (IP 85.253.120.25)
  с подпиской на **все** типы апдейтов — все сообщения пользователей уходили ему,
  а наш polling переставал получать что-либо;
- подменил **имя бота** (метод `setMyName`);
- подменил **меню команд** (`/start` → «✅ BEST CASINO MINI-APP @Xstakerobot ✅»).

Важно: **код бота на сервере не был изменён**, сервер не был взломан.
Всё было сделано снаружи через обычный Bot API — токена для этого достаточно.

## Как утёк токен (root cause)
Боевой токен лежал открытым текстом в файле **`.env.example`**, закоммиченном
в **публичный** GitHub-репозиторий `zhurbarv-hub/KKT`, начиная с **08.12.2025**
(коммит `b77a8ea`) — восемь месяцев в открытом доступе.
Публичные репозитории непрерывно сканируются ботами, которые собирают токены
и учётные данные автоматически.

Тем же путём утекли (найдено при разборе):
| Секрет | Где лежал | Статус |
|---|---|---|
| Токен Telegram-бота | `.env.example` | ⚠️ требуется отзыв в BotFather |
| `DB_PASSWORD` (kkt_user) | `KKT_TECH_DOC.md`, `_scripts_archive/quick_restore_test.sh` | ротация |
| `BOT_API_PASSWORD` (bot-service@kkt-system.ru) | `KKT_TECH_DOC.md`, `docker-compose.yml` | ✅ ротирован 08.08.2026 |
| Тестовый токен бота | `DEPLOYMENT_STEP2_COMPLETE.md` | вычищен |
| `JWT_SECRET_KEY` | — | ✅ не утекал |

## Что сделано
1. **Удалён чужой webhook**, восстановлены имя бота и меню команд.
2. **Вычищены все боевые секреты** из репозитория (коммит `b3759a5`), заменены плейсхолдерами.
3. **Ротирован пароль сервисной учётки** `bot-service@kkt-system.ru` — она была
   эксплуатируема прямо из интернета через `POST https://kkt-box.net/api/auth/login`.
4. **Права `600`** на все файлы `.env*` (раньше `.env.bak`, `.env.docker`,
   `.env.backup_*` были `644` — читаемы любым пользователем сервера).
5. **`.githooks/pre-commit`** — блокирует коммиты с токенами, паролями и приватными
   ключами. Включён через `core.hooksPath=.githooks`.
6. **`bot/services/integrity.py`** — самозащита бота (см. ниже).
7. Починен циклический перезапуск бота (см. ниже).

## Самозащита бота: `bot/services/integrity.py`
Сверяет фактические настройки бота в Telegram с эталонными и восстанавливает их:

| Проверка | Действие при расхождении |
|---|---|
| `getWebhookInfo().url` не пуст | webhook удаляется (`drop_pending_updates=True`) |
| имя бота ≠ `BOT_EXPECTED_NAME` | возвращается эталонное (по умолчанию «KKT System») |
| меню команд ≠ эталона | восстанавливается |
| описание / краткое описание не пусты | очищаются |

Запускается **при старте бота до polling** и далее **каждые 15 минут** из планировщика
(`bot/scheduler.py` → `integrity_watchdog`). При обнаружении подмены шлёт алерт всем
администраторам из `telegram_admin_ids` с инструкцией по отзыву токена.

Эталонное имя переопределяется переменной `BOT_EXPECTED_NAME` в `.env`.

⚠️ Это **компенсирующая мера**: она чинит последствия, но пока токен не отозван,
злоумышленник может ставить webhook снова и снова (максимум на 15 минут).

## Попутно найдено и исправлено: бот перезапускался каждые 10 секунд
`kkt-bot.service` содержал `Requires=kkt-web.service`, а `kkt-web.service` запускает
**несуществующий модуль** `web.app.main:app` (каталог `web/` давно стал фронтендом
на Vite, бэкенд живёт в `backend/` и обслуживается `kkt-backend.service`).
Юнит-призрак крэшился каждые 10 секунд — **NRestarts=169093** — и каждый раз
systemd глушил зависящего от него бота.

Исправлено: зависимость `kkt-bot.service` переключена на `kkt-backend.service`
(бэкап: `/etc/systemd/system/kkt-bot.service.bak_20260808`).
После этого бот работает стабильно, `NRestarts=0`.

**Остаётся сделать:** отключить мёртвый юнит —
```bash
systemctl stop kkt-web && systemctl disable kkt-web
```

## Правила работы с секретами (чтобы не повторилось)
1. Реальные значения — **только в `.env`** (он в `.gitignore`, права `600`).
2. В `.env.example` — **только плейсхолдеры**, никогда реальные значения.
3. В документации и отчётах (`*.md`) паролей быть не должно — писать
   `<DB_PASSWORD из .env>`.
4. В `docker-compose.yml` не задавать дефолты секретов через `${VAR:-реальное_значение}` —
   использовать `${VAR:?переменная не задана}`.
5. Перед первым пушем нового репозитория проверять, что он **приватный**.
6. Утёкший секрет считается скомпрометированным навсегда — его **ротируют**,
   а не «убирают из файла»: в истории git и в кэше GitHub он остаётся.
...
Исправлено: строка заменена на `EnvironmentFile=/home/kktapp/kkt-system/.env`,
теперь единственный источник секретов — `.env` (права 600).
Бэкап юнита: `/etc/systemd/system/kkt-backend.service.bak_20260808`.

**Вывод на будущее:** при смене любого секрета проверять не только `.env`, но и
`Environment=` в systemd-юнитах (`grep -l '<секрет>' /etc/systemd/system/*.service`).
Проверка «сайт отвечает 401» недостаточна — она не трогает БД; проверять нужно
путём, который реально идёт в базу (например, перезапуск бота и строка
`✅ Токен успешно обновлён` в его логах).

### Замечания, оставленные без изменений
- `kkt-backend.service` работает под `User=root`. Стоит перевести на `kktapp`
  (потребуется выверить права на `logs/`, `backups/` и т.п.).
- Дашборд показывает «0 клиентов, 0 активных дедлайнов», хотя в БД 34 пользователя
  и 168 дедлайнов — это поведение было и до работ 08.08.2026, к ротации отношения
  не имеет. Требует отдельного разбора фильтров в `/api/dashboard/stats`.
